(function (global, factory) {
    ...
    function createNameParslet({ allowedAdditionalTokens, allowReservedWords, module, strictMode, asyncFunctionBody }) {
        return composeParslet({
            name: 'nameParslet',
            ...
            parsePrefix: parser => {
                ...
                if (!allowReservedWords) {
                    if (reservedWords$1.always.includes(text)) {
    ...
    const createPathGrammars = ({ module, strictMode, asyncFunctionBody }) => {
    ...
    const createJsdocGrammars = ({ module, strictMode, asyncFunctionBody, }) => {
    ...
    const createClosureGrammars = ({ module, strictMode, asyncFunctionBody, }) => {
        const { pathGrammar } = createPathGrammars({
            module,
            strictMode,
            asyncFunctionBody,
    ...
    const createTypescriptGrammars = ({ module, strictMode, asyncFunctionBody, }) => {
        const { pathGrammar } = createPathGrammars({
            module,
            strictMode,
            asyncFunctionBody,
     ...
     * @param mode
     */
    function parse(expression, mode, { computedPropertyParser, module = true, strictMode = false, asyncFunctionBody = false } = {}) {
        switch (mode) {
            case 'closure': {
            ...
            case 'typescript': {
                const { typescriptGrammar } = createTypescriptGrammars({
                    module,
                    strictMode,
                    asyncFunctionBody
                });
     ...
     * @param mode
     */
    function parseNamePath(expression, mode, { module = true, strictMode = true, asyncFunctionBody = true } = {}) {
        switch (mode) {
            case 'closure': {
            ...
            case 'typescript': {
                const { typescriptNamePathGrammar } = createTypescriptGrammars({
                    module,
                    strictMode,
                    asyncFunctionBody
                });
                return (new Parser(typescriptNamePathGrammar, Lexer.create(rules, expression))).parse();
            }
        }
    }
    /**
     ...
     * @param mode
     */
    function parseName(expression, mode, { module = true, strictMode = true, asyncFunctionBody = true } = {}) {
        switch (mode) {
            case 'closure': {
            ...
            case 'typescript': {
                const { typescriptNameGrammar } = createTypescriptGrammars({
                    module,
                    strictMode,
                    asyncFunctionBody
                });
