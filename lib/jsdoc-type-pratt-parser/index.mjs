...
var NoParsletFoundError = class NoParsletFoundError extends Error {
	token;
	constructor(token) {
...
var EarlyEndOfParseError = class EarlyEndOfParseError extends Error {
	token;
	constructor(token) {
...
var UnexpectedTypeError = class UnexpectedTypeError extends Error {
...
const strictModeNonIdentifiers = ["arguments", "eval"];
//#endregion
//#region src/assertTypes.ts
...
function assertNumberOrVariadicNameResult(result) {
	if (result.type === "JsdocTypeVariadic") {
		if (result.element?.type === "JsdocTypeName") return result;
...
//#endregion
//#region src/parslets/isQuestionMarkUnknownType.ts
function isQuestionMarkUnknownType(next) {
	return next === "}" || next === "EOF" || next === "|" || next === "," || next === ")" || next === ">";
}
//#endregion
//#region src/parslets/NullableParslets.ts
const nullableParslet = (parser, precedence, left) => {
	const type = parser.lexer.current.type;
	const next = parser.lexer.next.type;
	if (!(left === null && type === "?" && !isQuestionMarkUnknownType(next) || left !== null && type === "?" && 12 > precedence)) return null;
	parser.consume("?");
	if (left === null) return {
...
//#endregion
//#region src/parslets/Parslet.ts
...
//#endregion
//#region src/parslets/OptionalParslet.ts
...
//#endregion
//#region src/parslets/NumberParslet.ts
...
//#endregion
//#region src/parslets/ParenthesisParslet.ts
...
//#endregion
//#region src/parslets/SpecialTypesParslet.ts
...
//#endregion
//#region src/parslets/NotNullableParslet.ts
...
//#endregion
//#region src/parslets/ParameterListParslet.ts
...
//#endregion
//#region src/parslets/GenericParslet.ts
	...
	parseInfix: (parser, left) => {
		...
		do
			if (parser.consume("infer")) {
				const name = parser.parseIntermediateType(10);
...
//#endregion
//#region src/parslets/UnionParslets.ts
...
//#endregion
//#region src/grammars/baseGrammar.ts
const baseGrammar = [
	nullableParslet,
	optionalParslet,
	numberParslet,
...
//#endregion
//#region src/parslets/NamePathParslet.ts
function createNamePathParslet({ allowSquareBracketsOnAnyType, allowJsdocNamePaths, pathGrammar }) {
	return function namePathParslet(parser, precedence, left) {
		...
		const pathParser = brackets && allowSquareBracketsOnAnyType ? parser : pathGrammar !== null ? new Parser(pathGrammar, parser.lexer, parser) : parser;
		const parsed = pathParser.parseType(18);
		...
		switch (parsed.type) {
			...
			case "JsdocTypeNumber":
				...
				break;
			case "JsdocTypeBigInt": throw new UnexpectedTypeError(parsed, "Expecting 'JsdocTypeName', 'JsdocTypeNumber', 'JsdocStringValue' or 'JsdocTypeSpecialNamePath'");
...
//#endregion
//#region src/parslets/NameParslet.ts
...
//#endregion
//#region src/parslets/StringValueParslet.ts
...
//#endregion
//#region src/parslets/SpecialNamePathParslet.ts
...
//#endregion
//#region src/grammars/pathGrammar.ts
...
//#endregion
//#region src/parslets/FunctionParslet.ts
...
//#endregion
//#region src/parslets/VariadicParslet.ts
...
//#endregion
//#region src/parslets/SymbolParslet.ts
...
//#endregion
//#region src/parslets/ArrayBracketsParslet.ts
...
//#endregion
//#region src/parslets/ObjectParslet.ts
function createObjectParslet({ signatureGrammar, objectFieldGrammar, allowKeyTypes }) {
	return composeParslet({
		...
		parsePrefix: (parser) => {
			...
			if (!parser.consume("}")) {
				let separator;
				const fieldParser = new Parser(objectFieldGrammar, parser.lexer, parser, parser.externalParsers?.computedPropertyParser !== void 0 ? { externalParsers: { computedPropertyParser: parser.externalParsers.computedPropertyParser } } : void 0);
				while (true) {
...
//#endregion
//#region src/parslets/ObjectFieldParslet.ts
...
//#endregion
//#region src/parslets/KeyValueParslet.ts
...
//#endregion
//#region src/grammars/jsdocGrammar.ts
...
const jsdocNamePathSpecialGrammar = [createSpecialNamePathParslet({
	allowedTypes: [
		"module",
		"external",
		"event"
...
//#endregion
//#region src/parslets/TypeOfParslet.ts
...
//#endregion
//#region src/grammars/closureGrammar.ts
...
const closureNamePathSpecialGrammar = [createSpecialNamePathParslet({
	allowedTypes: ["module"],
	pathGrammar
}), ...closureNamePathGrammar];
//#endregion
//#region src/parslets/assertsParslet.ts
...
//#endregion
//#region src/parslets/FunctionPropertyParslet.ts
		...
		if (parser.consume("new")) result = {
			...
			if (parser.consume("Identifier")) result = {
				...
				if (parser.consume("StringValue")) result = {
					type: "JsdocTypeMethodSignature",
					name: text.slice(1, -1),
					meta: { quote: text.startsWith("\"") ? "double" : "single" },
					parameters: [],
					returnType
				};
				else result = {
...
//#endregion
//#region src/parslets/TupleParslet.ts
...
//#endregion
//#region src/parslets/KeyOfParslet.ts
...
//#endregion
//#region src/parslets/ImportParslet.ts
...
//#endregion
//#region src/parslets/ReadonlyPropertyParslet.ts
...
//#endregion
//#region src/parslets/ArrowFunctionParslet.ts
...
//#endregion
//#region src/parslets/GenericArrowFunctionParslet.ts
...
//#endregion
//#region src/parslets/IntersectionParslet.ts
...
//#endregion
//#region src/parslets/predicateParslet.ts
...
var Lexer = class Lexer {
	...
	static create(lexerRules, text) {
		...
		text = next.text;
		return new Lexer(lexerRules, text, void 0, current.token, next.token);
	}
...
const objectSquaredPropertyParslet = composeParslet({
	...
	parsePrefix: (parser) => {
		...
		let result;
		if (innerBracketType?.type === "JsdocTypeObjectField" && typeof innerBracketType.key === "string" && !innerBracketType.optional && !innerBracketType.readonly && innerBracketType.right !== void 0) {
			...
			result.right = right;
			parser.acceptLexerState(parentParser);
		} else if (innerBracketType?.type === "JsdocTypeName" && parser.consume("in")) {
			const parentParser = parser.baseParser;
			parentParser.acceptLexerState(parser);
...
//#endregion
//#region src/parslets/ReadonlyArrayParslet.ts
...
//#endregion
//#region src/parslets/ConditionalParslet.ts
...
//#endregion
//#region src/lexer/LexerRules.ts
...
//#endregion
//#region src/parslets/TemplateLiteralParslet.ts
...
//#endregion
//#region src/parslets/UniqueSymbolParslet.ts
const uniqueSymbolParslet = (parser, _precedence, left) => {
...
//#endregion
//#region src/grammars/typescriptGrammar.ts
...
const typescriptNamePathSpecialGrammar = [createSpecialNamePathParslet({
	allowedTypes: ["module"],
	pathGrammar
...
function parse(expression, mode, { range = false, rangeStart, loc = false, locStart = {
	column: 0,
	line: 1
}, module = true, strictMode = true, asyncFunctionBody = true, classContext = false, computedPropertyParser } = {}) {
	let parser;
	switch (mode) {
		case "closure":
...
function parseNamePath(expression, mode, { includeSpecial = false } = {}) {
	switch (mode) {
		case "closure": return new Parser(includeSpecial ? closureNamePathSpecialGrammar : closureNamePathGrammar, Lexer.create(looseRules, expression)).parse();
		case "jsdoc": return new Parser(includeSpecial ? jsdocNamePathSpecialGrammar : jsdocNamePathGrammar, Lexer.create(looseRules, expression)).parse();
		case "typescript": return new Parser(includeSpecial ? typescriptNamePathSpecialGrammar : typescriptNamePathGrammar, Lexer.create(rules, expression)).parse();
	}
}
/**
...
function parseName(expression, mode) {
	switch (mode) {
		case "closure": return new Parser(closureNameGrammar, Lexer.create(looseRules, expression)).parse();
		case "jsdoc": return new Parser(jsdocNameGrammar, Lexer.create(looseRules, expression)).parse();
		case "typescript": return new Parser(typescriptNameGrammar, Lexer.create(rules, expression)).parse();
	}
}
//#endregion
//#region src/transforms/transform.ts
...
//#endregion
//#region src/transforms/stringify.ts
	...
	return {
		...
		JsdocTypeFunction: (result, transform) => {
			if (!result.arrow) {
				...
				if (result.returnType !== void 0) stringified += `${result.meta?.preReturnMarkerSpacing ?? ""}:${result.meta?.postReturnMarkerSpacing ?? " "}${transform(result.returnType)}`;
				return stringified;
			} else {
				if (result.returnType === void 0) throw new Error("Arrow function needs a return type.");
				let stringified = `${result.typeParameters !== void 0 ? `<${result.typeParameters.map(transform).join(`,${result.meta?.typeParameterSpacing ?? " "}`)}>${result.meta?.postGenericSpacing ?? ""}` : ""}(${result.parameters.map(transform).join(`,${result.meta?.parameterSpacing ?? " "}`)})${result.meta?.preReturnMarkerSpacing ?? " "}=>${result.meta?.postReturnMarkerSpacing ?? " "}${transform(result.returnType)}`;
		...
		JsdocTypeTuple: (result, transform) => `[${result.elements.map(transform).join(`,${result.meta?.elementSpacing ?? " "}`)}]`,
		...
		JsdocTypeGeneric: (result, transform) => {
			...
			} else return `${transform(result.left)}${result.meta.dot ? "." : ""}<${result.elements.map(transform).join(`,${result.meta.elementSpacing ?? " "}`)}>`;
		},
		...
		JsdocTypeObjectField: (result, transform) => {
			...
			else return `${text}:${result.meta.postColonSpacing ?? " "}${transform(result.right)}`;
		},
		JsdocTypeJsdocObjectField: (result, transform) => `${transform(result.left)}: ${transform(result.right)}`,
		JsdocTypeKeyValue: (result, transform) => {
			let text = result.key;
			if (result.optional) text += `${result.meta?.postKeySpacing ?? ""}?${result.meta?.postOptionalSpacing ?? ""}`;
			else if (result.variadic) text = `...${result.meta?.postVariadicSpacing ?? ""}${text}`;
			else if (result.right !== void 0) text += result.meta?.postKeySpacing ?? "";
			if (result.right === void 0) return text;
			else return `${text}:${result.meta?.postColonSpacing ?? " "}${transform(result.right)}`;
		},
		...
		JsdocTypeUnion: (result, transform) => result.elements.map(transform).join(result.meta?.spacing === void 0 ? " | " : `${result.meta.spacing}|${result.meta.spacing}`),
		JsdocTypeUnknown: () => "?",
		...
		JsdocTypeTypeParameter: (result, transform) => `${transform(result.name)}${result.constraint !== void 0 ? ` extends ${transform(result.constraint)}` : ""}${result.defaultValue !== void 0 ? `${result.meta?.defaultValueSpacing ?? " "}=${result.meta?.defaultValueSpacing ?? " "}${transform(result.defaultValue)}` : ""}`,
		JsdocTypeCallSignature: (result, transform) => `${result.typeParameters !== void 0 ? `<${result.typeParameters.map(transform).join(`,${result.meta?.typeParameterSpacing ?? " "}`)}>${result.meta?.postGenericSpacing ?? ""}` : ""}(${result.parameters.map(transform).join(`,${result.meta?.parameterSpacing ?? " "}`)})${result.meta?.preReturnMarkerSpacing ?? ""}:${result.meta?.postReturnMarkerSpacing ?? " "}${transform(result.returnType)}`,
		JsdocTypeConstructorSignature: (result, transform) => `new${result.meta?.postNewSpacing ?? " "}${result.typeParameters !== void 0 ? `<${result.typeParameters.map(transform).join(`,${result.meta?.typeParameterSpacing ?? " "}`)}>${result.meta?.postGenericSpacing ?? ""}` : ""}(${result.parameters.map(transform).join(`,${result.meta?.parameterSpacing ?? " "}`)})${result.meta?.preReturnMarkerSpacing ?? ""}:${result.meta?.postReturnMarkerSpacing ?? " "}${transform(result.returnType)}`,
		JsdocTypeMethodSignature: (result, transform) => {
			const quote = result.meta.quote === "double" ? "\"" : result.meta.quote === "single" ? "'" : "";
			return `${quote}${result.name}${quote}${result.meta.postMethodNameSpacing ?? ""}${result.typeParameters !== void 0 ? `<${result.typeParameters.map(transform).join(`,${result.meta.typeParameterSpacing ?? " "}`)}>${result.meta.postGenericSpacing ?? ""}` : ""}(${result.parameters.map(transform).join(`,${result.meta.parameterSpacing ?? " "}`)})${result.meta.preReturnMarkerSpacing ?? ""}:${result.meta.postReturnMarkerSpacing ?? " "}${transform(result.returnType)}`;
		},
		JsdocTypeIndexedAccessIndex: (result, transform) => transform(result.right),
		JsdocTypeTemplateLiteral: (result, transform) => `\`${result.literals.slice(0, -1).map((literal, idx) => `${literal.replace(/`/gv, "\\`")}\${${transform(result.interpolations[idx])}}`).join("") + result.literals.slice(-1)[0].replace(/`/gv, "\\`")}\``,
		JsdocTypeComputedProperty: (result, transform) => {
			if (result.value.type.startsWith("JsdocType")) return `[${transform(result.value)}]`;
			else {
		...
		JsdocTypeComputedMethod: (result, transform) => {
			if (result.value.type.startsWith("JsdocType")) return `[${transform(result.value)}]${result.optional ? "?" : ""}${result.typeParameters !== void 0 ? `<${result.typeParameters.map(transform).join(`,${result.meta?.typeParameterSpacing ?? " "}`)}>${result.meta?.postGenericSpacing ?? ""}` : ""}(${result.parameters.map(transform).join(`,${result.meta?.parameterSpacing ?? " "}`)})${result.meta?.preReturnMarkerSpacing ?? ""}:${result.meta?.postReturnMarkerSpacing ?? " "}${transform(result.returnType)}`;
			else {
				if (computedPropertyStringifier === void 0) throw new Error("Must have a computed property stringifier");
				return `[${computedPropertyStringifier(result.value).replace(/;$/v, "")}](${result.parameters.map(transform).join(`,${result.meta?.parameterSpacing ?? " "}`)})${result.meta?.preReturnMarkerSpacing ?? ""}:${result.meta?.postReturnMarkerSpacing ?? " "}${transform(result.returnType)}`;
			}
		}
	};
}
...
//#endregion
//#region src/transforms/catharsisTransform.ts
const reservedWords = [
	"null",
	"true",
	"false",
...
const catharsisTransformRules = {
	...
	JsdocTypeSymbol: (result) => {
		...
		if (element?.type === "JsdocTypeName") value += element.value;
		else if (element?.type === "JsdocTypeNumber") value += element.value.toString();
...
//#endregion
//#region src/transforms/jtpTransform.ts
...
//#endregion
//#region src/transforms/identityTransformRules.ts
...
function _traverse(node, parentNode, property, index, onEnter, onLeave) {
	...
	for (const key of keysToVisit) {
		...
		if (value !== void 0) {
			if (Array.isArray(value)) for (const [index, element] of value.entries()) _traverse(element, node, key, index, onEnter, onLeave);
			else if (value !== null && typeof value === "object" && "type" in value) _traverse(value, node, key, void 0, onEnter, onLeave);
		}
	}
	onLeave?.(node, parentNode, property, index);
}
/**
...
//#endregion
export { catharsisTransform, identityTransformRules, jtpTransform, parse, parseName, parseNamePath, stringify, stringifyRules, transform, traverse, tryParse, visitorKeys };
