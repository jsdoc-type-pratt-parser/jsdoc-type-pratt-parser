...
function stringifyRules() {
  return {
    ...
    JsdocTypeObject: (result, transform2) => {
      ...
      (lbType && (separatorForSingleObjectField || result.elements.length > 1) ? "\n" + ((_c = result.meta.propertyIndent) != null ? _c : "") : "") + result.elements.map(transform2).join(
        result.meta.separator === "comma" ? ", " : lbType ? lbEnding + /* c8 ignore next -- Guard */
        ((_d = result.meta.propertyIndent) != null ? _d : "") : "; "
      ) + (separatorForSingleObjectField && result.elements.length === 1 ? result.meta.separator === "comma" ? "," : lbType ? lbEnding : ";" : "") + (lbType && result.elements.length > 1 ? "\n" : "")}}`;
    },
